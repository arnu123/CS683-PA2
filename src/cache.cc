...
{
    // Checking if exclusivity is maintained before accepting writeback in L2
    if (cache_type == IS_L2C && WQ.occupancy > 0)
    {
        // Check if block exists in L1 before accepting writeback
        if (!check_exclusivity(&WQ.entry[WQ.head]))
        {
            // Block exists in L1, drop the writeback
...
uint32_t CACHE::get_way(uint64_t address, uint32_t set)
{
    // Debug output
#ifdef DEBUG_CACHE
    cout << "[" << NAME << "] get_way: address=0x" << hex << address
         << " set=" << dec << set << " NUM_SET=" << NUM_SET << endl;
#endif

    // Validate input parameters
    if (!block)
    {
        cout << "[" << NAME << "] Error: Uninitialized cache blocks" << endl;
        return NUM_WAY;
    }

    if (set >= L2C_SET)
    {
        cout << "[" << NAME << "] Error: Invalid set " << set
             << " (NUM_SET=" << NUM_SET << ")" << endl;
        return NUM_WAY;
    }

    // Calculate tag based on cache type
    uint64_t tag = address >> LOG2_BLOCK_SIZE;
    if (cache_type == IS_L1I || cache_type == IS_L1D)
    {
        tag = tag & ((1ULL << (32 - LOG2_BLOCK_SIZE)) - 1);
    }
    if (cache_type == IS_L2C || cache_type == IS_LLC)
    {
        tag = tag & ((1ULL << (36 - LOG2_BLOCK_SIZE)) - 1);
    }
    ...
    if (cache_type == IS_L2C)
    {
        ...
        {
            // Validate block pointer before access
            if (&block[set][way] == nullptr)
            {
                cout << "[" << NAME << "] Error: Invalid block pointer at set="
                     << set << " way=" << way << endl;
                return NUM_WAY;
            }

            // Check for valid block with matching tag
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    if (!packet)
    {
        cout << "[" << NAME << "] Error: Null packet in fill_cache" << endl;
        assert(0);
        return;
    }

    // Check set and way bounds
    if (set >= NUM_SET)
    {
        cout << "[" << NAME << "] Error: Invalid set " << set << " in fill_cache. NUM_SET=" << NUM_SET
             << " cache_type=" << +cache_type << " packet->address=0x" << hex << packet->address
             << " packet->full_addr=0x" << packet->full_addr << dec << endl;
        assert(0);
        return;
    }
    ...
    if (way >= NUM_WAY)
    {
        cout << "[" << NAME << "] Error: Invalid way " << way << " in fill_cache. NUM_WAY=" << NUM_WAY << endl;
        assert(0);
        return;
    }

    // Validate block array is initialized
    if (!block)
    {
        cout << "[" << NAME << "] Error: Cache blocks not initialized in fill_cache" << endl;
        assert(0);
        return;
    }
    ...
    // For exclusive caches: When filling L2, check if block exists in L1 before accepting
    if (cache_type == IS_L2C && block[set][way].valid)
    {
        // Create eviction packet to send to L1
        PACKET eviction_packet;
        eviction_packet.fill_level = FILL_L1;
        eviction_packet.cpu = packet->cpu;
        eviction_packet.address = block[set][way].address;
        eviction_packet.full_addr = block[set][way].full_addr;
        eviction_packet.data = block[set][way].data;
        eviction_packet.ip = block[set][way].ip;
        eviction_packet.type = WRITEBACK;

        // Calculate correct set index for L1 caches
        uint32_t l1_set = 0;
        if (block[set][way].instruction)
        {
            l1_set = upper_level_icache[packet->cpu]->get_set(block[set][way].address);
            upper_level_icache[packet->cpu]->fill_cache(l1_set, way, &eviction_packet);
        }
        else
        {
            l1_set = upper_level_dcache[packet->cpu]->get_set(block[set][way].address);
            upper_level_dcache[packet->cpu]->fill_cache(l1_set, way, &eviction_packet);
        }
    }
    // Update block state
    ...
    // Set cache-specific flags for L2C
    if (cache_type == IS_L2C)
    {
        block[set][way].instruction = (packet->type == LOAD || packet->type == PREFETCH) && packet->instruction;
        block[set][way].translation = packet->type == PREFETCH_TRANSLATION ||
    ...
    {
        pf_fill++;
        if (cache_type == IS_L1D && block[set][way].pref_class < 5)
        {
            pref_filled[packet->cpu][block[set][way].pref_class]++;
        }
    }
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    {
        // Condition checking for L2 to L1 fill on L2 hit
        if (cache_type == IS_L2C && block[set][way].valid && (block[set][way].tag == packet_tag))
        {
            // Calculate correct set index for L1 caches
            uint32_t l1_set = 0;
            PACKET hit_packet;
            hit_packet.fill_level = FILL_L1;
            hit_packet.cpu = packet->cpu;
            hit_packet.address = block[set][way].address;
            hit_packet.full_addr = block[set][way].full_addr;
            hit_packet.data = block[set][way].data;
            hit_packet.ip = block[set][way].ip;
            hit_packet.type = packet->type;

            // Send to appropriate L1 cache with correct set index
            if (packet->instruction)
            {
                l1_set = upper_level_icache[packet->cpu]->get_set(block[set][way].address);
                upper_level_icache[packet->cpu]->fill_cache(l1_set, way, &hit_packet);
            }
            else
            {
                l1_set = upper_level_dcache[packet->cpu]->get_set(block[set][way].address);
                upper_level_dcache[packet->cpu]->fill_cache(l1_set, way, &hit_packet);
            }
        }
...
bool CACHE::check_exclusivity(PACKET *packet)
{
    // Only L2 needs to check exclusivity
    if (cache_type == IS_L2C)
    {
        // Validate pointers before access
        if (!upper_level_icache[packet->cpu] || !upper_level_dcache[packet->cpu])
        {
            cout << "Error: Null pointer access in check_exclusivity" << endl;
            assert(0);
            return false;
        }
        ...
        if (packet->instruction)
        {
            if (upper_level_icache[packet->cpu]->get_way(packet->address, set) != NUM_WAY)
                return false;
        }
        else
        {
            if (upper_level_dcache[packet->cpu]->get_way(packet->address, set) != NUM_WAY)
                return false;
        }
    }
    return true;
