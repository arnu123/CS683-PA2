...
public:
    ...
    void fill_cache(uint32_t set, uint32_t way, PACKET *packet) override
    {
        return;
    }
    ...
    uint32_t get_way(uint64_t address, uint32_t set) override
    {
        return UINT32_MAX;
    }
    uint32_t get_set(uint64_t address) override
    {
        return UINT32_MAX;
    }
